...
import (
	...
	timelineConsumer "twitter-clone/internal/mq/consumer"
)

func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "timeline event dlq replay failed: %v\n", err)
		os.Exit(1)
	}
}

func run() error {
	event := flag.String("event", "", "required event DLQ: created or deleted")
	limit := flag.Int("limit", 20, "maximum messages to inspect or replay (1-100)")
	execute := flag.Bool("execute", false, "declare dedicated Timeline recovery topology, confirm-publish eligible messages, and acknowledge them from the DLQ")
	...
	options := timelineConsumer.TimelineEventReplayOptions{
		Kind:           timelineConsumer.TimelineEventKind(strings.ToLower(strings.TrimSpace(*event))),
		Limit:          *limit,
		Execute:        *execute,
	...
	replayer, err := timelineConsumer.NewTimelineEventDLQReplayer(broker)
	if err != nil {
		return err
	}
