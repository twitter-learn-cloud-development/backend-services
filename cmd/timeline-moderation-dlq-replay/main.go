...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "timeline moderation dlq replay failed: %v\n", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	options := timelineConsumer.ModerationReplayOptions{
		Limit:          *limit,
		Execute:        *execute,
	...
	replayer, err := timelineConsumer.NewModerationDLQReplayer(broker)
	if err != nil {
		return err
	}
