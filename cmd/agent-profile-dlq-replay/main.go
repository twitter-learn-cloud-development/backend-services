...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "agent profile dlq replay failed: %v\n", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	execute := flag.Bool("execute", false, "publish eligible messages back to twitter.events and acknowledge them from the DLQ")
	maxReplays := flag.Int("max-replays", 1, "maximum replay attempts allowed per message (1-10)")
	operator := flag.String("operator", strings.TrimSpace(os.Getenv("DLQ_REPLAY_OPERATOR")), "operator identity; required with --execute")
	...
	options := agentConsumer.ContentEngagementReplayOptions{
		Limit:          *limit,
		Execute:        *execute,
	...
	replayer, err := agentConsumer.NewContentEngagementDLQReplayer(broker)
	if err != nil {
		return err
	}
